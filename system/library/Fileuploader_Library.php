...
#
# ======================================================================== #
include_once("fileupload/images.class.php");
class Fileuploader_Library {
    ...
    public function __construct($options = null) {
        return $this->initialize($options['inputs'], $options);
    }
     ...
     * @param $options {null, Array}
     */
    private function initialize($name, $options) { //print_r($options);
        // merge options
        ...
        if (isset($_FILES[$name])) {
            // set field input
            $this->field['input'] = $_FILES[$name]; //print_r($this->field['input']);
    ...
    private function uploadFiles() {
        ...
        $listInput = $this->field['listInput']; //print_r($this->field);
        
        if ($this->field['input']['name']) {
            // validate ini settings and some generally options
            $validate = $this->validate(); //print_r($validate);
            $data['isSuccess'] = true;
            ...
            if ($validate === true) {
                ...
                for($i = 0; $i < count($this->field['input']['name']); $i++) {
                    ...
                    if ($validateFile === true) {
                        if ($fileInList) {
                            $data['files'][] = $metas;
                            $data['files_name'] .= isset($metas['name']) ? $metas['name'].',' : '' ;
                ...
                if (!$data['hasWarnings']) {
                    foreach($data['files'] as $key => $file) {
                        if (move_uploaded_file($file['tmp_name'], $file['file'])) {
                            $img_crop =$file['file'];
                            $thumb_path =$img_crop;
                            $imageThumb =new Image($img_crop);
                            $imageThumb->createThumb($thumb_path,1024,768);
        ...
        $data['files_name'] = trim($data['files_name'],',');
    ...
    private function codeToMessage($code, $file = null) {
        ...
        switch ($code) {
            ...
            case 'invalid_folder_path':
                $message = "Thư mục upload không tồn tại, hoặc không có quyền truy cập";
                break;
            default:
